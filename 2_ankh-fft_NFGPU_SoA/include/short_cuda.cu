...
{
    cudaFree(d_leaves_nptr);
    // cudaFree(d_leaves_corresp);
    cudaFree(d_leaves_parts);
    cudaFree(d_leaves_Qs);
    // cudaFree(d_leaves_Ps);
    // cudaFree(d_leaves_ctr);
    cudaFree(d_res);   
}
...
void nfgpu::short_range_naiiveGPU_dataCollection(ankh::leaf<ankh::charge_dipole_quadrupole_3<double>>* leaves, int lvl, int Nimages, double diam)
{
    ...
    std::cout<<"max_nptr=" <<max_nptr<< " max_parts="<<max_parts<<" max_qs="<<max_qs<<" numBoxes="<<numBoxes<<" E = " << (1<<lvl) <<std::endl;

    //allocate memory
    int* leaves_nptr =(int*)malloc(numBoxes * sizeof(int)); 
    double* leaves_parts =(double*)malloc(numBoxes * max_parts * sizeof(ankh::part));
    double* leaves_Qs =(double*)malloc(numBoxes * max_qs * sizeof(double));
    int dataSize = numBoxes * (sizeof(int) + sizeof(double) * (max_qs + 1) + sizeof(ankh::part) * max_parts);
    ...
    for(int idx_trg_x = 0; idx_trg_x < E; idx_trg_x++)//for all boxes in 3d grid
    {
    	for(int idx_trg_y = 0; idx_trg_y < E; idx_trg_y++)
        {
			for(int idx_trg_z = 0; idx_trg_z < E; idx_trg_z++)
            {
                ...
                leaves_nptr[idx_trg] = leaves[idx_trg].nprt;
                for(int k=0; k<leaves[idx_trg].prts.size(); k++)
                {
                    leaves_parts[idx_trg * max_parts * 3 + k*3 + 0] = leaves[idx_trg].prts[k].x;
                    leaves_parts[idx_trg * max_parts * 3 + k*3 + 1] = leaves[idx_trg].prts[k].y;
                    leaves_parts[idx_trg * max_parts * 3 + k*3 + 2] = leaves[idx_trg].prts[k].z;
                }
                for(int k=0; k<leaves[idx_trg].Qs.size(); k++)
                {
                    leaves_Qs[idx_trg * max_qs + k] = leaves[idx_trg].Qs[k];
                }
			}
		}
    }
    ...
    cudaMalloc((void**)&d_leaves_nptr, numBoxes * sizeof(int)); 
    cudaMalloc((void**)&d_leaves_parts, numBoxes * max_parts * sizeof(ankh::part));
    cudaMalloc((void**)&d_leaves_Qs, numBoxes * max_qs * sizeof(double));
    cudaMalloc((void**)&d_res, numBoxes * sizeof(double));

    //copy to GPU
    cudaError_t er = cudaMemcpy(d_leaves_nptr, leaves_nptr, numBoxes * sizeof(int), cudaMemcpyHostToDevice);
    if(er != cudaSuccess){
        printf("error in copying d_leaves_nptr : %s\n", cudaGetErrorString(er));
    }
    er = cudaMemcpy(d_leaves_parts, leaves_parts, numBoxes * max_parts * sizeof(ankh::part), cudaMemcpyHostToDevice);
    if(er != cudaSuccess){
        printf("error in copying d_leaves_parts : %s\n", cudaGetErrorString(er));
    }
    er = cudaMemcpy(d_leaves_Qs, leaves_Qs, numBoxes * max_qs * sizeof(double), cudaMemcpyHostToDevice);
    if(er != cudaSuccess){
        printf("error in copying d_leaves_Qs : %s\n", cudaGetErrorString(er));
    }
    ...
    std::cout << "                         Near field - Memory Allocation (" << round((double)dataSize/1024.0/1024.0) <<" MB)" << std::endl;
}
double nfgpu::short_range_wrapper(int lvl, int Nimages, double diam)
{
    ...
    short_range_cuda<<<grid, block>>>(d_leaves_nptr, d_leaves_parts, d_leaves_Qs, max_parts, max_qs, d_res, Nimages, diam, E);
...
__global__ void short_range_cuda(int* d_leaves_nptr, double *d_leaves_parts, double *d_leaves_Qs, int max_parts, int max_qs, double* d_res, int Nimages, double diam, int length)
{
                    ...
                    if(Nimages == 0){
                        if(sidx > -1 && sidx < length && sidy > -1 && sidy < length && sidz > -1 && sidz < length){
                            tmp = direct_interaction_0(
                                d_leaves_nptr[idx_trg], &d_leaves_parts[idx_trg * max_parts * 3], &d_leaves_Qs[idx_trg * max_qs],
                                d_leaves_nptr[idx_src], &d_leaves_parts[idx_src * max_parts * 3], &d_leaves_Qs[idx_src * max_qs],
