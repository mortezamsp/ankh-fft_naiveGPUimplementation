...
namespace ankh{
  ...
  public :
    ...
    double apply(charge_type* q, double* parts, int Nm){
      ...
      // t0 = chronos::time();
      // double near_field1 = short_range<charge_type,KRNL>(grid,lvl,Nimages,2*box_rad);
      // t1 = chronos::time();
      // std::cout << "                         Near field (sequential): " << t1-t0 << std::endl;
      ////compute short range interactions on GPU
      t0 = chronos::time();
      nfgpu nfg(lvl);
      nfg.short_range_naiiveGPU_dataCollection(grid,lvl,Nimages, 2*box_rad);
      t1 = chronos::time();
      std::cout << "                         Near field - Data Allocation : " << t1-t0 << "s" << std::endl;
      t0 = chronos::time();
      double near_field = nfg.short_range_wrapper(lvl, Nimages, 2*box_rad);
      t1 = chronos::time();
      std::cout << "                         Near field - kernel execution: " << t1-t0 << "s" << std::endl;
      // Ewald correction
      t0 = chronos::time();
      ...
      //std::cout << "near_field=" << near_field << " near_field1=" << near_field1 << std::endl;
