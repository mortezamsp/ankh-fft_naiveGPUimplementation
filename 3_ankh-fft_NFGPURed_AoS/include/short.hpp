...
namespace ankh{
  ...
  template<>
  double direct_interaction<charge_dipole_3<double>,0>(leaf<charge_dipole_3<double> >& trg, leaf<charge_dipole_3<double> >& src, int Nimages, double diam){
    double res = 0.;
    for(int i = 0; i < trg.nprt; i++){
  ...
  template<typename charge_type, int KRNL>
  double direct_interaction_0(leaf<charge_type>& trg, leaf<charge_type>& src, int u0, int u1, int u2, double diam, bool print){
    std::cout << "direct_interaction_0 not implemented for the general case" << std::endl;}

  template<>
  double direct_interaction_0<double,0>(leaf<double>& trg, leaf<double>& src, int u0, int u1, int u2, double diam, bool print){
    double res = 0.;
    for(int i = 0; i < trg.nprt; i++){
      for(int j = 0; j < src.nprt; j++){
  ...
  template<> 
  double direct_interaction_0<double,1>(leaf<double>& trg, leaf<double>& src, int u0, int u1, int u2, double diam, bool print)
  {
    double res = 0.;
    for(int i = 0; i < trg.nprt; i++){
		for(int j = 0; j < src.nprt; j++){
			...
			double dy = trg.prts[i].y - src.prts[j].y + u1*diam;
			double dz = trg.prts[i].z - src.prts[j].z + u2*diam;
			double tmp = energy<double,1,0>(dx,dy,dz,&(trg.Qs[i]),&(src.Qs[j]));
			res += tmp;
      }
    }
    return res;
  ...
  template<>//for evald kernel
  double direct_interaction_0<charge_dipole_quadrupole_3<double>,1>(leaf<charge_dipole_quadrupole_3<double> >& trg, leaf<charge_dipole_quadrupole_3<double> >& src, int u0, int u1, int u2, double diam, bool print)
  {
    double res = 0.;
    for(int i = 0; i < trg.nprt; i++){
		for(int j = 0; j < src.nprt; j++){
			...
			double dy = trg.prts[i].y - src.prts[j].y + u1*diam;
			double dz = trg.prts[i].z - src.prts[j].z + u2*diam;
			double tmp = energy<double,1,2>(dx,dy,dz,&(trg.Qs[10*i]),&(src.Qs[10*j]));
			res += tmp;
			//if(print && j==src.nprt-1) printf("%f ", src.Qs[10*i]);//tmp);
      }
    }
    return res;
  }
  ...
  template<>
  double direct_interaction_0<charge_dipole_3<double>,0>(leaf<charge_dipole_3<double> >& trg, leaf<charge_dipole_3<double> >& src, int u0, int u1, int u2, double diam, bool print){
    double res = 0.;
    for(int i = 0; i < trg.nprt; i++){
      double tq   = trg.Qs[4*i  ];
      double tmu0 = trg.Qs[4*i+1];
  ...
  template<typename charge_type, int KRNL> //the function used in analyze test : ankh::charge_dipole_quadrupole_3<double>, 1:EWALD (where 0:coulomb)
    ...
    for(int idx_trg_x = 0; idx_trg_x < E; idx_trg_x++){//for all boxes in 3d grid
    	for(int idx_trg_y = 0; idx_trg_y < E; idx_trg_y++){
			for(int idx_trg_z = 0; idx_trg_z < E; idx_trg_z++){
				int idx_trg = idx_in_grid(idx_trg_x,idx_trg_y,idx_trg_z,E); //sequential number of box in 3d grid
				for(int idx_src_x = -1; idx_src_x <= 1; idx_src_x++){ //for all E2 neighbors in any dierction in 3d grid
					for(int idx_src_y = -1; idx_src_y <= 1; idx_src_y++){
						for(int idx_src_z = -1; idx_src_z <= 1; idx_src_z++){
							...
							int idx_src = idx_in_grid(sidx,sidy,sidz,E); //sequential number of box in 3d grid
							if(Nimages == 0){
								if(sidx > -1 && sidx < E && sidy > -1 && sidy < E && sidz > -1 && sidz < E){
									// res += direct_interaction_0<charge_type,KRNL>(leaves[idx_trg],leaves[idx_src],0,0,0,diam); //direct kenel call
									double tmp = direct_interaction_0<charge_type,KRNL>(leaves[idx_trg],leaves[idx_src],0,0,0,diam, false); 
									res += tmp;
									if(idx_trg == 0) printf("N%f,",tmp);
								}
							}
							else{ //applying bounday condition
								...
								// res += direct_interaction_0<charge_type,KRNL>(leaves[idx_trg], leaves[idx_src], u0,u1,u2,diam);
								double tmp = 0;
								if(idx_trg == 0 && idx_src == 1) 
								tmp = direct_interaction_0<charge_type,KRNL>(leaves[idx_trg], leaves[idx_src], u0,u1,u2,diam, true);
								else
								tmp = direct_interaction_0<charge_type,KRNL>(leaves[idx_trg], leaves[idx_src], u0,u1,u2,diam, false);
								res += tmp;
								//if(idx_trg == 0) printf("B%f,",tmp);
							}
						}
					}
				}
